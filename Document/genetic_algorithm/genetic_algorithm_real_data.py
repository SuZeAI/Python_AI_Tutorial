...
def generate_data():
    ...
    """
    coeff = [0.5, -0.4, 0.3, -0.2, 0.1]
    x = [[random() for j in range(len(coeff))] for i in range(1000)]
    for i in range(1000):
        x[i][-1] = 1
    e = np.random.normal(0, 0.05, 1000)
    y = [dot(i, coeff) for i in x] + e 
...
print('Multiple_linear_regression execution time: ' )
print('from: ')
print(start_time)
print('to: ')
print(end_time)
...
start_time = datetime.now() # for time statistic
